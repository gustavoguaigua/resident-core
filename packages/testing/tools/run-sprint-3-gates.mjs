...
const evidencePath = resolve(
  repositoryRoot,
  process.env.SPRINT3_EVIDENCE_PATH ?? "artifacts/sprint-3-gates/evidence.json",
);
const evidence = {
  schemaVersion: 1,
...
const runPnpmScript = (script) => {
  if (!rootPackage.scripts?.[script]) {
    ...
    writeEvidence();
    throw new Error(`Active Sprint 3 gate script is missing: ${script}.`);
  }
...
runPnpmScript("sprint3:boundary");
...
writeEvidence();
process.stdout.write(`Sprint 3 gate evidence: ${evidencePath}\n`);
