...
const postgresContainer = `resident-phase6-postgres-${suffix}`;
const minioContainer = `resident-phase6-minio-${suffix}`;
const postgresPort = 56627;
const minioPort = 59003;
const password = `synthetic-${randomBytes(24).toString("base64url")}`;
const accessKey = `phase6${randomBytes(8).toString("hex")}`;
const secretKey = `synthetic-${randomBytes(24).toString("base64url")}`;
const databaseUrl = `postgresql://phase6:${password}@127.0.0.1:${postgresPort}/phase6?schema=public`;
const environment = {
  ...process.env,
  APP_ENV: "local",
  DATABASE_URL: databaseUrl,
  PAYMENTS_PHASE6_TEST: "1",
  DOCUMENT_STORAGE_PROVIDER: "MINIO",
  S3_ACCESS_KEY: accessKey,
  S3_BUCKET: "resident-phase6-private",
...
try {
  ...
  for (let attempt = 0; attempt < 60; attempt += 1) {
    if ((await connectable(postgresPort)) && (await connectable(minioPort)))
      break;
    if (attempt === 59)
      throw new Error("Phase 6 services did not become ready.");
  ...
  run(process.execPath, [
    vitestCli,
    "run",
    "apps/api/test/integration/payments-receipts.integration.spec.ts",
  ]);
  process.stdout.write(
    `${JSON.stringify({ gate: "test:payments", minio: "RELEASE.2025-09-07T16-13-09Z", postgres: "17.10-bookworm", status: "PASS" })}\n`,
  );
} finally {
  if (started) {
