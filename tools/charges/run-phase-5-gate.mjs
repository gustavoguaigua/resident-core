...
const port = 56626;
const container = "resident-core-sprint3-phase5-charges";
const password = `synthetic-${randomBytes(24).toString("base64url")}`;
const databaseUrl = `postgresql://resident_phase5:${password}@127.0.0.1:${port}/resident_phase5?schema=public`;
...
try {
  ...
  run(docker, [
    ...
    "--env",
    "POSTGRES_DB=resident_phase5",
    "--env",
    "POSTGRES_USER=resident_phase5",
    "--env",
  ...
  for (let attempt = 0; attempt < 60; attempt += 1) {
    ...
    if (attempt === 59)
      throw new Error("Phase 5 PostgreSQL did not become ready.");
  ...
  run(process.execPath, [
    vitestCli,
    "run",
    "apps/api/test/integration/charge-lifecycle.integration.spec.ts",
  ]);
  process.stdout.write(
    `${JSON.stringify({ gate: "test:charges", postgres: "17.10-bookworm", status: "PASS" })}\n`,
  );
} finally {
  if (started) cleanup();
}
