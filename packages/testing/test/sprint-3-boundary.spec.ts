...
describe("Sprint 3 boundary verifier", () => {
  it("accepts the current GO phase 4 boundary", () => {
    const result = runVerifier();

    expect(result.status).toBe(0);
    expect(result.stdout).toContain(
      "Sprint 3 boundary is valid at phase 4 (GO); applicable documents: 35, needs-review: 0.",
    );
  });

  it("rejects a future Sprint 3 Prisma model during phase 4", () => {
    ...
    expect(result.stderr).toContain(
      "Sprint 3 phase 4 forbids domain models: Charge.",
    );
  });

  it("rejects a Sprint 3 OpenAPI path during phase 4", () => {
    const directory = createTemporaryDirectory();
    ...
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain(
      "Sprint 3 phase 4 forbids functional API operations: GET /api/v1/tenant/persons.",
    );
  });
