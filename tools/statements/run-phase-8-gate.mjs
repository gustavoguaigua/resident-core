...
const container = `resident-phase8-statements-${suffix}`;
const port = 56629;
...
try {
  ...
  const services = exec(docker, ["compose", "config", "--services"], "pipe");
  if (
    services.status !== 0 ||
  ...
  run(process.execPath, [
    vitest,
    "run",
    "apps/api/test/integration/account-statements.integration.spec.ts",
  ]);
  process.stdout.write(
    `${JSON.stringify({ gate: "test:statements", postgres: "17.10-bookworm", status: "PASS" })}\n`,
  );
} finally {
  if (started) remove();
}
